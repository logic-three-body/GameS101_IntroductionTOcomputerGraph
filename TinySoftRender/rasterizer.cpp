...
void rasterizer::DrawModelFrame(Model & model, Vec3f light_dir)
{
	DrawModelFrame(model, width, height, light_dir);
}

void rasterizer::DrawModelFrame(Model & model, int width, int height, Vec3f light_dir)
{
	for (int i = 0; i < model.nfaces(); i++) {
		std::vector<int> face = model.face(i);
		Vec3f screen_coords[3];
